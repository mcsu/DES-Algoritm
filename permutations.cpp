...
char* f(char *data,char *k,int fois)//一个32位的数据区块和一个48位的秘钥区块Kn 特别难
{
    ...
    cout<<"expand:"<<endl;
    displayBitTable(exData,48);
    displayAsHex(exData,48);

    char *xorData = new char[48];
    xorData = xOR(exData,k,48);
    cout<<"xOR:"<<endl;
    displayBitTable(xorData,48);
    displayAsHex(xorData,48);
...
//s-box addr
    ...
    cout<<"Sbox output "<<endl;
    displayAsHex(sBoxData,32);
    char* finalResult = permute(sBoxData,permut_32,32);
    cout<<"after 32"<<endl;
    displayAsHex(finalResult,32);
    delete[]sBoxData;
...
void Iteration(char *l, char *r, int level)
{
    cout<<endl;
    cout<<"ROUND "<<level+2<<endl;
    cout<<"L"<<level+1<<endl;
    ...
    cout<<"used key:"<<endl;
    displayBitTable(kn[level+1],48);
    displayAsHex(kn[level+1],48);
    char *ln,*rn;
    ln=r;

    rn=xOR(l,f(r,kn[level+1],0),32);

    cout<<"xOR output"<<endl;
    displayBitTable(rn,32);
    displayAsHex(rn,32);

    if(level<14)
    {
        level++;
        Iteration(ln,rn,level);
    }

    else
    {
        char* reser = reserve(ln,rn,32);
